...
//     image: "/images/categories/vacuumCleaner-transparent.webp",
//     href: "/products/vacuum-cleaner"
//   },
//   {
//     title: "Sewing Machines",
//     image: "/images/categories/sewingMachine-transparent.webp",
//     href: "/products/sewing-machine"
//   },
...
export default function Navbar() {
  ...
  return (
    <header className={`fixed top-0 start-0 z-50 w-full transition-[background-color] duration-300 ease-linear ${dropHov || isScrolled || !isHome ? "bg-white shadow-md" : "bg-transparent"}`}>
      <div className="flex max-w-[1655px] mx-auto items-center justify-between px-4 md:px-12 xl:px-16 py-5.5">
        <Image
          ...
          height={24}
          priority
          className={`filter transition-[filter] duration-300 ease-linear ${!isMutationPage && (dropHov || isScrolled || !isHome) ? "brightness-100 invert-0" : "lg:brightness-0 lg:invert"}`}
        />
        <div
        ...
        <div className="flex items-center gap-6">
          {/* Update the dropdown part of the navigation: */}
          <ul className={`hidden 2xl:flex ${isMutationPage && !dropHov && !isScrolled ? "text-white" : "text-black"} gap-6`}  dir={dir}>
            {t.items.map(item => (
              <li
                key={item.title}
          ...
          <div className="hidden 2xl:block">
            <LanguageSwitcher lang={lang} isDark={(isMutationPage && !dropHov && !isScrolled) ? true : false} />
          </div>
        </div>
      </div>
      <div className={`absolute bg-white min-h-screen min-w-56 px-2 py-5  z-50 top-0 overflow-y-auto h-screen transition-[inset-inline-start] duration-300 ease-out 2xl:hidden ${isOpen ? "start-0" : "-start-full"}`}>
        <Image
          src={jantechLogo}
          width={150}
          height={150}
